...
def test_agent_endpoint_invalid_transaction():
    response = client.post(
        "/api/recovery/agent/999999"
    )
...
def test_agent_endpoint_valid_transaction():
    response = client.post(
        "/api/recovery/agent/1003"
    )

    # Either 200 (analysis generated) or 429 (external Gemini quota limit)
    ...
    if response.status_code == 200:
        ...
        assert "rate limit" in response.json()["detail"].lower() or "quota" in response.json()["detail"].lower()